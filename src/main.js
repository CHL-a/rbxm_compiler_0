...
class lz4_struct{
	/**
	 * @param {RBXM_stream} from 
	 * @param {RBXM_stream} to 
	 */
	constructor(from,to){
	...
	constructor(parent){
		...
		collection[0x03] = /** @type {RBXM_helper<number>} */
			(new RBXM_helper(parent)).set_extract(len => {
				...
				result.forEach(
					(v,i) => result[i]=RBXM_stream.detransform_int(v)
				)
 ...
 */
class RBXM_stream extends bitStream.BitstreamReader{
	/**
	 * @param {Buffer} b 
	 */
	constructor(){
...
class RBXM{
	...
	constructor(buffer){
		...
		/** @type {Object.<number, RBXM_class_struct>} */
		this.class_structs = {}
